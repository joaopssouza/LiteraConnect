...
- **Uso:** Autenticação (JWT), tabelas relacionais (`users`, `follows`, `posts` metadados), e orquestração de **Realtime** (Chat e Notificações).
...
- **Cloudinary:** Responsável pelo upload, otimização e entrega via CDN de avatares, capas de livros e vídeos (Reels futuros).
